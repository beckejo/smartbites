...
class _WeightMeasurementScreenState extends State<WeightMeasurementScreen> {
  ...
  void _calculateWeightUsed() {
    ...
    if (scaleAmountUsed != null) {
      // Round to 1 decimal place before returning
      Navigator.pop(context, (scaleAmountUsed! * 10).round() / 10);
      return;
    }
    
    // Otherwise use manual calculations
    final initialWeight = double.tryParse(initialWeightController.text) ?? 0.0;
    final finalWeight = double.tryParse(finalWeightController.text) ?? 0.0;
    final weightUsed = (initialWeight - finalWeight).toDouble();

    // Round to 1 decimal place before returning
    Navigator.pop(context, (weightUsed * 10).round() / 10);
  }
